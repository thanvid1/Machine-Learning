...
bc_model=DecisionTreeClassifier(random_state=42)
...
sample_data=pd.DataFrame({
    ...
    'communication':['good','moderate','poor','good','moedrate','moderate','poor','good','good','good'],
...
plot_tree(sample_model,filled=True,feature_names=X_sample.columns, class_names=label_encoders['job_offer'].classes_, rounded=True)
plt.title("Decision Tree for Job Offer Prediction")
plt.show()

test_sample=pd.DataFrame([{
    'cgpa':6.5,'interactiveness':'yes','practical_knowledge':'good','communication':'good'
}])
